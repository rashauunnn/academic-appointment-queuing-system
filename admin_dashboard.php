...
try {
    ...
    // 3. Recent Activity Log (Actions like booking, completing)
...
<body class="font-sans antialiased text-slate-100 min-h-screen">
    ...
    <nav class="glass sticky top-0 z-50 px-6 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
            <div class="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-800/50 border border-slate-700/50">
                <div class="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]"></div>
                <p class="text-xs font-bold text-slate-300">System Live</p>
            </div>
            <p class="text-sm font-bold text-slate-200"><?php echo htmlspecialchars($admin_name); ?></p>
    ...
    <main class="max-w-7xl mx-auto px-6 py-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
            <div class="glass p-7 rounded-[2.5rem] hover:scale-[1.02] transition-transform cursor-pointer group">
                ...
                <p class="text-4xl font-bold text-white"><?php echo $stats['total_students']; ?></p>
                <div class="mt-4 flex items-center gap-1 text-green-400 text-xs font-bold">
            ...
            <div class="glass p-7 rounded-[2.5rem] hover:scale-[1.02] transition-transform cursor-pointer group">
                ...
                <p class="text-4xl font-bold text-white"><?php echo $stats['total_faculty']; ?></p>
            ...
            <div class="glass p-7 rounded-[2.5rem] border-indigo-500/30 ring-4 ring-indigo-500/5 hover:scale-[1.02] transition-transform cursor-pointer group">
                <p class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-3 group-hover:text-indigo-300 transition-colors">Today's Apps</p>
                <p class="text-4xl font-bold text-indigo-400"><?php echo $stats['apps_today']; ?></p>
                <div class="mt-4 flex items-center gap-1 text-indigo-300 text-xs font-bold">
            ...
            <div class="glass p-7 rounded-[2.5rem] hover:scale-[1.02] transition-transform cursor-pointer group">
                ...
                <p class="text-4xl font-bold text-green-400"><?php echo $stats['completed_sessions']; ?></p>
        ...
        <div class="mb-12">
            <div class="flex items-center gap-3 mb-6 px-2">
                ...
                <h2 class="text-xl font-bold uppercase tracking-tight text-slate-300">Peak Traffic Analysis</h2>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="peak-card p-6 rounded-[2rem] flex items-center justify-between group hover:border-indigo-500/50 transition-all cursor-default">
                    <div>
                        <p class="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-1">Rank #<?php echo $index+1; ?></p>
                        <p class="font-bold text-slate-100"><?php echo formatHour($peak['hr']); ?></p>
                    </div>
                    <div class="text-right">
                        <p class="text-2xl font-black text-white"><?php echo $peak['count']; ?></p>
                        <p class="text-[10px] font-bold text-slate-500 uppercase">Bookings</p>
                    </div>
                </div>
                <?php endforeach; ?>
                <?php if (empty($peak_hours)): ?>
                    <div class="col-span-3 py-10 text-center text-slate-500 italic glass rounded-[2rem]">No traffic data available yet. Start booking to see analytics.</div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div>
                <div class="flex items-center justify-between mb-6 px-2">
                    <h2 class="text-xl font-bold">Recent System Activity</h2>
                ...
                <div class="glass rounded-[2.5rem] p-5">
                    <div class="space-y-3">
                        ...
                        <div class="p-4 rounded-2xl bg-slate-900/40 border border-slate-700/30 flex items-center justify-between group hover:border-slate-500 hover:bg-slate-800/40 transition-all">
                            <div class="flex items-center gap-4">
                                <div class="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center text-slate-500 shrink-0 group-hover:scale-110 transition-transform">
                                ...
                                <div class="overflow-hidden">
                                    <p class="text-sm font-bold text-slate-200 truncate">
                                        <span class="text-indigo-400"><?php echo htmlspecialchars($act['student_name']); ?></span> 
                                        <?php 
                                        ...
                                        ?> 
                                        <span class="text-violet-400"><?php echo htmlspecialchars($act['faculty_name']); ?></span>
                                    </p>
                        ...
                        <?php if (empty($recent_activity)): ?>
                            <div class="py-16 text-center text-slate-500">No activity logs found. System is waiting for interaction.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div class="flex items-center justify-between mb-6 px-2">
                    <h2 class="text-xl font-bold">Faculty Availability Monitor</h2>
                    <button class="text-[10px] font-black text-indigo-400 uppercase tracking-widest hover:text-white transition-colors">Refresh Radar</button>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-800/50">
                            ...
                            <tr class="hover:bg-slate-800/40 transition-colors group">
                                <td class="px-8 py-5 border-transparent">
                                    <p class="text-sm font-bold text-slate-200 group-hover:text-white transition-colors"><?php echo htmlspecialchars($fac['full_name']); ?></p>
                                    <p class="text-[10px] text-slate-500 font-mono tracking-tighter uppercase mt-0.5 group-hover:text-slate-400"><?php echo htmlspecialchars($fac['school_id']); ?></p>
                                </td>
                                <td class="px-8 py-5">
                                    <div class="flex items-center gap-3">
                                        <div class="w-2.5 h-2.5 rounded-full ring-4 ring-opacity-10 <?php 
                                            echo $fac['current_status'] === 'Available' ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.4)] ring-green-500' : 
                                                ($fac['current_status'] === 'On Break' ? 'bg-yellow-500 ring-yellow-500' : 'bg-red-500 ring-red-500'); 
                                        ?>"></div>
                                        <span class="text-xs font-black uppercase tracking-wider <?php 
                                            echo $fac['current_status'] === 'Available' ? 'text-green-400' : 
                                                ($fac['current_status'] === 'On Break' ? 'text-yellow-400' : 'text-red-400'); 
                                        ?>">
                ...
                <div class="mt-8 p-10 rounded-[3rem] bg-gradient-to-br from-indigo-600/10 via-slate-800/50 to-violet-600/10 border border-indigo-500/20 relative overflow-hidden group">
                    <div class="absolute top-0 right-0 p-8 scale-150 opacity-10 group-hover:opacity-20 transition-opacity">
                         <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>
                    </div>
                    <h3 class="font-black text-xl mb-3 tracking-tight">Intelligence Oversight</h3>
                    <p class="text-sm text-slate-400 leading-relaxed mb-8">
                        ...
                        No anomalies detected in the last 24-hour cycle.
                    </p>
                    <div class="flex flex-col sm:flex-row gap-4">
                        <button class="flex-1 py-4 px-6 rounded-2xl bg-white/5 hover:bg-white/10 text-xs font-black uppercase tracking-widest transition-all border border-white/5">Broadcast Notice</button>
                        <button class="flex-1 py-4 px-6 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-indigo-600/20">Audit Database</button>
                    </div>
                </div>
            </div>
        </div>
